...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "FOLNetwork") type = FOLNetwork;
    if(strType == "FOLNetworkVariable") type = FOLNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == FOLNetwork) s = "FOLNetwork";
    if(type == FOLNetworkVariable) s = "FOLNetworkVariable";
